...
export default function MapView({ 
  ...
  // Functie om een locatienaam op te zoeken op basis van coördinaten
  const getLocationName = (latitude: number, longitude: number): string => {
    // Mapping van bekende Nederlandse steden op basis van coördinaten
    const locations = [
    ...
    const closest = locations.reduce((best: any, location) => {
    ...
    // Als er geen specifieke stad is gevonden, genereer dan een algemene naam op basis van provincie
    const provincies = [
    ...
    const closestProvincie = provincies.reduce((best: any, provincie) => {
      const distance = calculateDistance(latitude, longitude, provincie.lat, provincie.lng);
      if (!best || distance < best.distance) {
        return { ...provincie, distance };
      }
      return best;
    }, null);
    
    if (closestProvincie) {
      return `Regio ${closestProvincie.name}`;
    }
