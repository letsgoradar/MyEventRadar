...
export function AppFavoritesPage() {
  ...
  const handleNavigateEvent = React.useCallback((direction: 'previous' | 'next') => {
    ...
    const currentIndex = favoriteEvents.findIndex(e => e.id === selectedEvent.id);
    if (direction === 'previous' && currentIndex > 0) {
      setSelectedEvent(favoriteEvents[currentIndex - 1]);
    } else if (direction === 'next' && currentIndex < favoriteEvents.length - 1) {
      setSelectedEvent(favoriteEvents[currentIndex + 1]);
    }
  }, [selectedEvent, favoriteEvents]);
